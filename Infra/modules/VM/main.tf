...
resource "azurerm_public_ip" "lb" {
  ...
  name                = "${var.environment}-${local.tier_name}-pip"
...
resource "azurerm_lb" "backend" {
  ...
  name                = "${var.environment}-${local.tier_name}-lb"
  location            = var.location
  resource_group_name = var.resource_group_name
  sku                 = "Standard"
...
resource "azurerm_lb_backend_address_pool" "backend" {
  count           = var.is_frontend ? 0 : 1
  name            = "${var.environment}-${local.tier_name}-backend-pool"
  loadbalancer_id = azurerm_lb.backend[0].id
}

resource "azurerm_lb_probe" "backend" {
  count               = var.is_frontend ? 0 : 1
  name                = "${var.environment}-${local.tier_name}-probe"
  loadbalancer_id     = azurerm_lb.backend[0].id
  protocol            = "Http"
  port                = var.application_port
...
resource "azurerm_lb_rule" "backend" {
  count                          = var.is_frontend ? 0 : 1
  name                           = "${var.environment}-${local.tier_name}-rule"
  loadbalancer_id                = azurerm_lb.backend[0].id
  protocol                       = "Tcp"
...
resource "azurerm_user_assigned_identity" "vmss_identity" {
  name                = "${var.environment}-${local.tier_name}-identity"
  resource_group_name = var.resource_group_name
  location            = var.location
...
resource "azurerm_linux_virtual_machine_scale_set" "vmss" {
  name                = "${var.environment}-${local.tier_name}-vmss"
  resource_group_name = var.resource_group_name
  location            = var.location
  sku                 = var.vm_size
...
resource "azurerm_monitor_autoscale_setting" "vmss_autoscale" {
  name                = "${var.environment}-${local.tier_name}-autoscale"
  resource_group_name = var.resource_group_name
  location            = var.location
